...
struct command commands[] = {
  {"accept_secret_chat", {ca_secret_chat, ca_none}, do_accept_secret_chat, "accept_secret_chat <secret_chat>\t\t\tAccepts secret chat. Only useful with -E option"},
  {"add_contact", {ca_string, ca_string, ca_string, ca_none}, do_add_contact, "add_contact <phone> <first_name> <last_name>\t\tTries to add user to contact list"},
  ...
  {"search", {ca_peer | ca_optional, ca_number | ca_optional, ca_number | ca_optional, ca_number | ca_optional, ca_number | ca_optional, ca_string_end}, do_search, "search [peer] [limit] [from] [to] [offset] <pattern>\tSearch for pattern in messages from date to date (unixtime) in messages with peer (if peer not present, in all messages)"},
