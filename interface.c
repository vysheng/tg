...
void do_help (int arg_num, struct arg args[], struct in_ev *ev) {
  ...
  while (cmd->name) {
    mprintf (ev, "%-*s %s\n", COMMAND_DESC_COLUMN, cmd->usage, cmd->desc);
    cmd ++;
  }
  mpop_color (ev);
  if (ev) { mprint_end (ev); }
...
struct command commands[] = {
  ...
  {"secret_chat_rekey", { ca_secret_chat, ca_none}, do_secret_chat_rekey, "secret_chat_rekey", "Generates new key for active secret chat"},
